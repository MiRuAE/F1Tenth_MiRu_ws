...
private:
  ...
  void image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
  {
    ...
    if (lane_count == 2) {
      cv::circle(visFrame, cv::Point(filtered_lane_center, height - 1), 10, cv::Scalar(0, 0, 255), -1);
    }
    else if (lane_count == 1) {
      cv::circle(visFrame, cv::Point(filtered_lane_center, height - 1), 10, cv::Scalar(0, 255, 0), -1);
    }
    else {
      cv::circle(visFrame, cv::Point(filtered_lane_center, height - 1), 10, cv::Scalar(255, 0, 0), -1);
    }
    ...
    if (fabs(steering_degree) <= 5.0) { 
        drive_speed = 1.5;
    } else if (fabs(steering_degree) <= 10.0) {
        drive_speed = 1.0;
