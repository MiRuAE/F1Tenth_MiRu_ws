...
private:
    ...
    std::vector<float> preprocess_lidar(const std::vector<float>& ranges) {
        double scan_threshold = 2.0 + (current_speed_ - 0.8) * 2;  // 속도에 따라 동적으로 scan_threshold 설정
    ...
    void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        current_steering_angle_ = msg->pose.pose.orientation.y;  // 예시로 y축 사용, 실제는 차량에 맞게 조정 필요
    ...
    void lidar_callback(const sensor_msgs::msg::LaserScan::SharedPtr scan_msg) {
        ...
        int start_idx_main = static_cast<int>((-(M_PI / 2.0) - scan_msg->angle_min) / scan_msg->angle_increment);
        int end_idx_main = static_cast<int>(((M_PI / 2.0) - scan_msg->angle_min) / scan_msg->angle_increment);
        int start_idx_left = static_cast<int>(((M_PI / 2.0) - scan_msg->angle_min) / scan_msg->angle_increment); // 90
        int end_idx_left = static_cast<int>((scan_msg->angle_max - scan_msg->angle_min) / scan_msg->angle_increment); // 135
        int start_idx_right = static_cast<int>((scan_msg->angle_min - scan_msg->angle_min) / scan_msg->angle_increment); // -135
        int end_idx_right = static_cast<int>((-(M_PI / 2.0) - scan_msg->angle_min) / scan_msg->angle_increment); // -90

        double left_safety_distance = 0.2;  // 왼쪽 안전 거리 (예: 0.1m)
        double right_safety_distance = 0.2;  // 오른쪽 안전 거리 (예: 0.1m)

        bool can_turn_left = true;
        bool can_turn_right = true;

        for (int i = start_idx_left; i <= end_idx_left; ++i) {
            if (scan_msg->ranges[i] < left_safety_distance) {
                can_turn_left = false;  // 왼쪽으로 회전할 수 없음
                break;
            }
        }
        
        for (int i = start_idx_right; i <= end_idx_right; ++i) {
            if (scan_msg->ranges[i] < right_safety_distance) {
                can_turn_right = false;  // 오른쪽으로 회전할 수 없음
                break;
            }
        }

        std::vector<float> filtered_ranges(scan_msg->ranges.begin() + start_idx_main, scan_msg->ranges.begin() + end_idx_main + 1);
        
        // 필터링된 라이다 데이터를 전처리
        filtered_ranges = preprocess_lidar(filtered_ranges);
        
        // 불연속점 찾기
        std::vector<int> discontinuities = find_discontinuities(filtered_ranges, 0.1);  // threshold를 기본값으로 설정
        
        // 불연속점이 없다면, 가장 큰 값들 중에서 중앙값을 선택
        if (discontinuities.empty()) {
            double max_value = *std::max_element(filtered_ranges.begin(), filtered_ranges.end());
            std::vector<int> max_indices;
            ...
            // 가장 큰 값들의 중앙 인덱스를 선택
            int central_index = max_indices[max_indices.size() / 2];
            auto drive_msg = ackermann_msgs::msg::AckermannDriveStamped();
            double steering_angle = scan_msg->angle_min + (central_index + start_idx_main) * scan_msg->angle_increment;
            if (!can_turn_left && steering_angle > 0) {
                steering_angle = 0;  // 왼쪽으로는 돌 수 없으므로 오른쪽으로만
            } else if (!can_turn_right && steering_angle < 0) {
                steering_angle = 0;  // 오른쪽으로는 돌 수 없으므로 왼쪽으로만
            }
            drive_msg.drive.steering_angle = steering_angle;
            double drive_speed = 1.2;  // 기본 속도 설정
            drive_msg.drive.speed = drive_speed;
            RCLCPP_INFO(this->get_logger(), "No Gap, heading %d", central_index + start_idx_main);
            drive_pub_->publish(drive_msg);
            return;  // 더 이상 불연속점을 처리할 필요 없음
        }

        // 불연속점이 있으면 기존의 갭 찾기 로직 사용
        apply_bubbles(filtered_ranges, discontinuities, 0.2, scan_msg->angle_increment);  // 차량 반경 0.2m로 가정
        ...
        // 최종 스티어링 각도 계산
        double steering_angle = scan_msg->angle_min + (closest_index + start_idx_main) * scan_msg->angle_increment;
        if (!can_turn_left && steering_angle > 0) {
            steering_angle = 0;  // 왼쪽으로는 돌 수 없으므로 오른쪽으로만
        } else if (!can_turn_right && steering_angle < 0) {
            steering_angle = 0;  // 오른쪽으로는 돌 수 없으므로 왼쪽으로만
        ...
        RCLCPP_INFO(this->get_logger(), "Following Gap, heading %d", closest_index + start_idx_main);
        drive_pub_->publish(drive_msg);
    }
};
